...
void * xmalloc_edge(size_t size)
{
	void * p;
	if (size == 0) {
		std::cerr << "xmalloc: zero size" << std::endl;
	}
	p = malloc(size);
	if (p == NULL) {
		std::cerr << "xmalloc: out of memory" << std::endl;;
	}
	return p;
}

/* compute a > b considering the rounding errors due to the representation of double numbers*/
bool greate(double a, double b)
{
...
void gaussian_kernel(double * pfGaussFilter, int n, double sigma, double mean)
{
	...
	double val;
	int i;
	if (pfGaussFilter == NULL) {
		std::cerr << "gaussian_kernel: kernel not allocated" << std::endl;
	}
	if (sigma <= 0.0) {
		std::cerr<<"gaussian_kernel: sigma must be positive"<<std::endl;
	}
...
void gaussian_filter(uchar* image, uchar* out, int X, int Y, double sigma)
{
	...
	if (sigma <= 0.0) {
		std::cerr << "gaussian_filter: sigma must be positive" << std::endl;;
	}
	if (image == NULL || X < 1 || Y < 1) {
		std::cerr << "gaussian_filter: invalid image" << std::endl;
	}


	tmp = (double *)xmalloc_edge(X * Y * sizeof(double));
	prec = 3.0;
	offset = (int)ceil(sigma * sqrt(2.0 * prec * log(10.0)));
	n = 1 + 2 * offset; //kernel size
	kernel = (double *)xmalloc_edge(n * sizeof(double));
...
double chain(int from, int to, double * Ex, double * Ey,double * Gx, double * Gy, int X, int Y)
{
	...
	if (Ex == NULL || Ey == NULL || Gx == NULL || Gy == NULL) {
		std::cerr << "chain: invalid input" << std::endl;
	}
	if (from < 0 || to < 0 || from >= X * Y || to >= X * Y) {
		std::cerr << "chain: one of the points is out the image" << std::endl;
	}
...
void compute_gradient(double * Gx, double * Gy, double * modG,uchar * image, int X, int Y)
{
	...
	if (Gx == NULL || Gy == NULL || modG == NULL || image == NULL) {
		std::cerr << "compute_gradient: invalid input" << std::endl;
	}
	...
	double * Gx, double * Gy, int X, int Y)
{
	...
	if (Ex == NULL || Ey == NULL || modG == NULL || Gx == NULL || Gy == NULL) {
		std::cerr << "compute_edge_points: invalid input" << std::endl;
	}
	...
	for (y = 2; y<(Y - 2); y++)
	{
		...
		if (greate(mod, L) && !greate(R, mod) && gx >= gy) {
			Dx = 1;
		}
		else if (greate(mod, D) && !greate(U, mod) && gx <= gy) Dy = 1; /* V */
...
void chain_edge_points(int * next, int * prev, double * Ex,	double * Ey,double * Gx, double * Gy, int X, int Y)
{
	...
	if (next == NULL || prev == NULL || Ex == NULL || Ey == NULL || Gx == NULL || Gy == NULL) {
		std::cerr << "chain_edge_points: invalid input" << std::endl;
	}
	...
	double * modG,	int X, int Y,double th_h, double th_l)
{
	...
	if (next == NULL || prev == NULL || modG == NULL) {
		std::cerr << "thresholds_with_hysteresis: invalid input" << std::endl;
	}
		

	/* get memory */
	valid = (int *)xmalloc_edge(X * Y * sizeof(int));
	...
	int * M,int * next, int * prev,	double * Ex, double * Ey, int X, int Y)
{
	...
	*/
	*x = (double *)xmalloc_edge(X * Y * sizeof(double));
	*y = (double *)xmalloc_edge(X * Y * sizeof(double));
	*curve_limits = (int *)xmalloc_edge(X * Y * sizeof(int));
	*N = 0;
	*M = 0;
...
{
	double * Gx = (double *)xmalloc_edge(X * Y * sizeof(double));     /* grad_x */
	double * Gy = (double *)xmalloc_edge(X * Y * sizeof(double));     /* grad_y */
	double * modG = (double *)xmalloc_edge(X * Y * sizeof(double));   /* |grad| */
	double * Ex = (double *)xmalloc_edge(X * Y * sizeof(double));     /* edge_x */
	double * Ey = (double *)xmalloc_edge(X * Y * sizeof(double));     /* edge_y */
	int * next = (int *)xmalloc_edge(X * Y * sizeof(int));			 /* next point in chain */
	int * prev = (int *)xmalloc_edge(X * Y * sizeof(int));			 /* prev point in chain */
